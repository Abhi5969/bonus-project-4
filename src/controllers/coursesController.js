...
const getAllCourse = async (req, res) => {
  const getAllCourse = await courseModel.find({isDeleted:false}).select({_id:0,__v:0,caeatedAt:0,updataedAt:0})
  res.status(200).json({ msg: `All courses`,data:getAllCourse});
};
const getSingleCourse = async (req, res) => {
  const id=req.params.id
  const getSingleCourse= await courseModel.findById(id).select({_id:0,__v:0,caeatedAt:0,updataedAt:0})
...
const createCourse = async (req, res) => {
  ...
  if(!description) res.status(400).send({message:`description is require !`})
...
const updateCourse = async (req, res) => {
  ...
  let updateCourse = await courseModel.findOneAndUpdate({_id:id,isDeleted:false},data,{new:true})
  res.status(200).json({ msg: `Update course`,data:updateCourse });
};
const deleteCourse = async (req, res) => {
  const id=req.params.id
  let deleteCourse = await courseModel.findByIdAndUpdate(id,{$set:{isDeleted:true}})
